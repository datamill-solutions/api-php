...
/**
 * FirstNameApi Class Doc Comment
 *
 * @category Class
 * @package  Swagger\Client
 * @author   http://github.com/swagger-api/swagger-codegen
...
class FirstNameApi
{
     ...
     * @return \Swagger\Client\Model\FirstNameGetResponse
     */
     ...
     * @param string $name Full name to detect all first names and extract them (required)
     * @throws \Swagger\Client\ApiException on non-2xx response
     * @return array of \Swagger\Client\Model\FirstNameGetResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function getFirstNamesWithHttpInfo($license, $guid, $name)
    {
        ...
        if ((strlen($license) > 29)) {
            throw new \InvalidArgumentException('invalid length for "$license" when calling FirstNameApi.getFirstNames, must be smaller than or equal to 29.');
        }
        if ((strlen($license) < 29)) {
            throw new \InvalidArgumentException('invalid length for "$license" when calling FirstNameApi.getFirstNames, must be bigger than or equal to 29.');
        }
        ...
        if ((strlen($guid) < 30)) {
            throw new \InvalidArgumentException('invalid length for "$guid" when calling FirstNameApi.getFirstNames, must be bigger than or equal to 30.');
        }
        ...
        if ((strlen($name) > 255)) {
            throw new \InvalidArgumentException('invalid length for "$name" when calling FirstNameApi.getFirstNames, must be smaller than or equal to 255.');
        }
        if ((strlen($name) < 3)) {
            throw new \InvalidArgumentException('invalid length for "$name" when calling FirstNameApi.getFirstNames, must be bigger than or equal to 3.');
        }
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Swagger\Client\Model\FirstNameGetResponse',
                '/firstname/get'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Swagger\Client\Model\FirstNameGetResponse', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Swagger\Client\Model\FirstNameGetResponse', $e->getResponseHeaders());
