...
class GenderApi
{
     ...
     * @param string $firstname First name to recognize gender. (required)
     * @throws \DataMill\ApiException on non-2xx response
     * @return array of \\GenderGetResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function getGenderWithHttpInfo($license, $guid, $firstname)
    {
        ...
        try {
            ...
            return [$this->apiClient->getSerializer()->deserialize($response, '\\GenderGetResponse', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\\GenderGetResponse', $e->getResponseHeaders());
     ...
     * @throws \DataMill\ApiException on non-2xx response
     * @return \\GenderExtendedGetResponse
     */
     ...
     * @param string $countrycode ISO 3166-1 alpha-2 country code e.g. &#39;US&#39;. Please see https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2 for further information. (required)
     * @throws \DataMill\ApiException on non-2xx response
     * @return array of \\GenderExtendedGetResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function getGenderExtendedWithHttpInfo($license, $guid, $firstname, $countrycode)
    {
        ...
        try {
            ...
            return [$this->apiClient->getSerializer()->deserialize($response, '\\GenderExtendedGetResponse', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\\GenderExtendedGetResponse', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                case 401:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\\ErrorUnauthorized', $e->getResponseHeaders());
